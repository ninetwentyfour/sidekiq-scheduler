...
require 'sidekiq/scheduler/lock'
...
module Sidekiq
  ...
  private
    ...
    def build_master_lock
      if supports_lua?
        Sidekiq::Scheduler::Lock::Resilient.new(master_lock_key)
      else
        Sidekiq::Scheduler::Lock::Basic.new(master_lock_key)
      end
    end
    ...
    def redis_master_version
      Sidekiq.redis.info['redis_version'].to_f
    end
  end
end
